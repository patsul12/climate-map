...
/***/ function(module, exports, __webpack_require__) {
	...
	var projection = geo.geoAlbers().translate([width / 2, height / 2]).scale([1000]);

	var path = geo.geoPath(projection);

	var svg = d3.select("body").append("svg").attr("witdth", width).attr("height", height);

	d3.json("./stateOutlines.json", function (json) {
	  svg.selectAll("path").data(json.features).enter().append("path").attr("d", path).style("stroke", "#fff").style("stroke-width", "1");
	});
